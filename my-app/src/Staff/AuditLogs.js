...
export default function AuditLogs() {
  ...
  // Filter logs to staff role only (case-insensitive)
    ...
    <div className="admin-main">
      <div className="admin-header">
        <div className="header-content">
          <h1>Audit Logs</h1>
          <p>View staff activities and actions</p>
        </div>
        <button className="btn-add" onClick={fetchLogs} disabled={loading}>↻ Refresh</button>
      </div>
            ...
            {filteredLogs.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" style={{ textAlign: 'center', padding: '20px', color: '#6b7280' }}>
                  {loading ? 'Loading audit logs...' : 'No audit logs found for staff role'}
                </td>
              </tr>
            )}
          </tbody>
