...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      try {
        ...
        // Pre-fill current password if it exists on the stored user object
        // (Note: this mirrors existing app behavior where login response is stored in localStorage)
        setPasswordData(prev => ({
          ...prev,
          currentPassword: '' // Do not pre-fill for security - user must type current password
        }));
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (data.success && data.user) {
        const updatedUser = { ...user, ...formData, name: fullName, ...data.user };
        localStorage.setItem('user', JSON.stringify(updatedUser));
        setUser(updatedUser);
  ...
  // Change password (saved to login table via API)
  const handleChangePassword = async () => {
    ...
    try {
      ...
      try {
        ...
        setMessage({
          type: 'error',
          text: `Server returned non-JSON (status ${res.status}). Restart the backend and try again.${preview ? ` Response: ${preview}…` : ''}`
        });
        setSaving(false);
        return;
      }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      const fullName = [formData.first_name, formData.last_name].filter(Boolean).join(' ').trim();
      // Save profile to database when user is logged in and required fields are present
      if (user?.id && formData.first_name && formData.last_name && formData.email) {
        const res = await fetch('/api/auth/update-profile', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (data.user) {
          const updatedFromDb = { ...user, ...formData, name: fullName, ...data.user, preferences };
          localStorage.setItem('user', JSON.stringify(updatedFromDb));
          setUser(updatedFromDb);
        } else {
          const updatedUser = { ...user, ...formData, preferences, name: fullName || user?.name };
          localStorage.setItem('user', JSON.stringify(updatedUser));
          setUser(updatedUser);
        }
      } else {
  ...
  return (
    <div className="user-page settings-page">
      <UserSidebar active="settings" />
      <main className="user-main settings-main">
        <div className="settings-header">
          ...
          <button
            ...
            disabled={saving}
          >
            {saving ? <ReloadIcon style={{ width: '16px', height: '16px', verticalAlign: 'middle', marginRight: '4px', animation: 'spin 1s linear infinite' }} /> : <ReloadIcon style={{ width: '16px', height: '16px', verticalAlign: 'middle', marginRight: '4px' }} />} {saving ? 'Refreshing...' : 'Refresh'}
          </button>
        </div>
          ...
          {activeTab === 'profile' && (
            <div className="settings-section">
              ...
              <div className="logged-in-as">
                Logged in as: <strong>{(user?.name || `${formData.first_name} ${formData.last_name}`.trim() || user?.email || 'Unknown')}</strong>
              </div>
