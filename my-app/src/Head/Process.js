...
import './HeadPage.css';
import { ReloadIcon, Cross2Icon, CheckIcon, FileIcon } from '@radix-ui/react-icons';
import { LayoutGrid, List, LayoutDashboard, TrendingUp, Activity, Users, MessageCircle, Settings, ShieldCheck } from 'lucide-react';
...
export default function Process() {
    ...
    // Backend serves uploads on port 5000; frontend may be on 5001 — resolve so iframe/link load the file
    ...
    // Helper to parse attachments from database
    ...
    const isV2Attachment = (att) => {
        if (!att) return false;
        const name = getAttachmentDisplayName(att, typeof att === 'string' ? att : (att.url || att.path || att.filePath || att.attachment_url || att.attachment || ''));
        return /_V2(?=\.[^.]+$)/i.test(name);
    };
    ...
    const recordAuditLog = async (action, description, userRole = 'User') => {
        try {
            ...
            if (stored) {
                try {
                    ...
                    userEmail = parsed.name || parsed.email || userEmail;
                } catch (e) { }
            }
    ...
    // Fetch received_by list and communications from database
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Extract unique received_by values from communications
    ...
    const handleApprove = async () => {
        ...
        setIsApproving(true);
        try {
            const res = await fetch(`/api/communications/approve/${selected.id}`, {
                method: 'PUT'
            });
            ...
            recordAuditLog(
                'PROJECT_APPROVED',
                `Approved project/communication #${selected.id}${selected.subject ? `: "${selected.subject}"` : ''}`,
                'User'
            );
        } catch (err) {
    ...
    const handleReject = async () => {
        ...
        setIsRejecting(true);
        try {
            const res = await fetch(`/api/communications/reject/${selected.id}`, {
                method: 'PUT'
            });
            ...
            recordAuditLog(
                'PROJECT_REJECTED',
                `Rejected project/communication #${selected.id}${selected.subject ? `: "${selected.subject}"` : ''}`,
                'User'
            );
        } catch (err) {
    ...
    const selectedAttachmentsToShow = selected ? getFirstOriginalAttachment(selected.attachments) : [];

    return (
        <div className="head-page projects-page">
            ...
            <main className="head-main">
                <div className="page-header">
                    ...
                    <div className="page-header-filter">
                        <label style={{ fontSize: '12px', color: '#666', fontWeight: '600', textTransform: 'uppercase', marginRight: '8px' }}>Received By:</label>
                        <select value={receivedByFilter} onChange={(e) => setReceivedByFilter(e.target.value)} style={{ padding: '8px 10px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '13px', fontFamily: 'inherit', backgroundColor: '#fff', cursor: 'pointer', marginRight: '16px' }}>
                ...
                <section className="projects-list" style={{ marginTop: '-10px' }}>
                                    ...
                                    <div className="modal-backdrop" onClick={() => setShowModal(false)}>
                                        <div className="modal details-modal" onClick={e => e.stopPropagation()}>
                                            <div className="modal-header details-header">
                                                ...
                                                <button className="modal-close" onClick={() => setShowModal(false)}><Cross2Icon /></button>
                                            </div>
                                                ...
                                                {((selected.attachments && (Array.isArray(selected.attachments) ? selected.attachments.length : Object.keys(JSON.parse(selected.attachments || '{}')).length > 0)) || selected.attachment || selected.attachment_url) && (
                                                    <div className="detail-section attachments-section">
                                                        ...
                                                        <div className="attachments-grid">
                                                            {selectedAttachmentsToShow.length > 0 && selectedAttachmentsToShow.map((a, i) => {
                                                                const href = a.url || a.path || a;
                                                                const name = getAttachmentDisplayName(a, href);
                                                                const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(href);
                                                                return (
                                                            ...
                                                            {selectedAttachmentsToShow.length === 0 && selected.attachment && (
                                                            ...
                                                            {selectedAttachmentsToShow.length === 0 && !selected.attachment && selected.attachment_url && (
                                                                <div className="attachment-card">
                                                                    <div className="attachment-icon">�</div>
                                                                    <div className="attachment-info">
                                        ...
                                        <div className="modal-backdrop" onClick={() => setShowAttachmentModal(false)}>
                                            <div className="modal attachment-modal" onClick={e => e.stopPropagation()}>
                                                <div className="modal-header">
                                                    ...
                                                    <button className="modal-close" onClick={() => setShowAttachmentModal(false)}><Cross2Icon /></button>
                                                </div>
                                                <div
                                                    style={{
                                                        padding: '12px 20px',
                                                ...
                                                >
                                                    <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                        <FileIcon style={{ width: '14px', height: '14px' }} /> {viewingAttachment.name}
                                                    </span>
                                                    <a
                                                        href={displayUrl}
                                                        target="_blank"
                                    ...
                                    <div className="modal-backdrop" onClick={() => setEditingAttachmentIndex(null)}>
                                        <div className="modal edit-modal" onClick={e => e.stopPropagation()}>
                                            <div className="modal-header">
                                                ...
                                                <button className="modal-close" onClick={() => setEditingAttachmentIndex(null)}><Cross2Icon /></button>
                                            </div>
